...
### Request-scoped сервисы и транзакционная модель

Бизнес-логика приложения построена вокруг паттерна **Unit of Work** и request-scoped контейнера сервисов (`ServiceManager`).

Жизненный цикл запроса выглядит следующим образом:

```
HTTP request
   └── ServiceManager (request-scoped)
         └── UnitOfWork
               └── AsyncSession (одна транзакция)
```
...
- используется **ровно один экземпляр `UnitOfWork`**;
- используется **ровно одна `AsyncSession` SQLAlchemy**;
- все операции внутри транзакции выполняются **строго последовательно**.

`AsyncSession` SQLAlchemy **не является coroutine-safe**.  
Это означает, что в пределах одного запроса запрещено:

- использовать `asyncio.gather(...)` для вызова сервисов;
- запускать `asyncio.create_task(...)`, использующий сервисы;
- передавать `ServiceManager`, сервисы или `UnitOfWork` в фоновые задачи;
- выполнять конкурентные операции, использующие одну и ту же сессию.

Все обращения к сервисам должны выполняться строго через последовательный `await`.

Нарушение этого инварианта может привести к ошибкам состояния транзакции, таким как:
...
Если требуется конкурентное выполнение операций, каждая задача должна:

- создавать собственный `UnitOfWork`;
- работать со своей `AsyncSession`;
- иметь независимую транзакцию.

Таким образом, конкурентность возможна **между транзакциями**, но не внутри одной.

### 💡 Почему так?

Транзакция в данном проекте рассматривается как:

- атомарная бизнес-операция;
- логически последовательная единица работы;
- граница согласованности данных.

Это осознанное архитектурное ограничение, а не технический компромисс.
